...
input <- "C:/Users/hhy270/Documents/GitHub/2019_DGE_Correlation_Oil_supplements_study/a_Data/1_DGE_analysis_(Edge_R)/"
...
# - Column "quality": specify which samples passed the quality control steps. 
...
# We have two options: Likelihood ratio test and quasi-likelihood F test. The consensus said that the second one
# is best. The first one is recommended for Single Cell RNA seq and experiments with not replicates.
...
edge_sig_all <- table_data_filter[(table_data_filter$PValue < 0.1), ] # A table with only the statistical significance genes. 
edge_sig_all$gene <- rownames(edge_sig_all)
edge_sig <- head(edge_sig_all, n = 10)
neg_edge_sig <- edge_sig_all[(edge_sig_all$logFC < 0) & (edge_sig_all$FDR < 0.1), ]
...
ggplot(table_data_filter, aes(logFC,-log10(PValue))) +  #LogFC as x and -log10(P.value) as y.  
  ...
  {if (nrow(edge_sig_pv) > 0) geom_point(data = edge_sig_pv[edge_sig_pv$logFC > 0, ], 
                                         aes(logFC,-log10(PValue)), color = "red3") } + # Color only significant genes. 
  {if (nrow(edge_sig_pv) > 0) geom_point(data = edge_sig_pv[edge_sig_pv$logFC < 0, ], 
